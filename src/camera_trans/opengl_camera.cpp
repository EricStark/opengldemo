...
int main() {
    ...
    Shader ourShader("/Users/joma/cpp_project/opengl_test/src/shaders/shader.vs", "/Users/joma/cpp_project/opengl_test/src/shaders/shader.fs");

    // Set up vertex data (and buffer(s)) and attribute pointers,这个数组表示一个立方体
    GLfloat vertices[] = {
    ...
    glBindTexture(GL_TEXTURE_2D, texture1); // All upcoming GL_TEXTURE_2D operations now have
    ...
    int width, height;
    unsigned char *image = SOIL_load_image("/Users/joma/cpp_project/opengl_test/assets/awesomeface.png", &width, &height, 0, SOIL_LOAD_RGB);
    if (image == nullptr) {
    ...
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
    // Load, create texture and generate mipmaps
    image = SOIL_load_image("/Users/joma/cpp_project/opengl_test/assets/awesomeface.png", &width, &height, 0, SOIL_LOAD_RGB);
    if (image == nullptr) {
        printf("Failed to load image: %s\n", SOIL_last_result());
    }
